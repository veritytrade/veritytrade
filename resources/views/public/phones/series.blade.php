<x-app-layout>
    @php
        $seriesOptions = $series->map(function ($item) {
            return [
                'id' => $item->id,
                'name' => $item->name,
                'image' => ($item->representative_image ?: $item->image_path) ? asset('storage/' . ($item->representative_image ?: $item->image_path)) : null,
                'devices' => $item->devices->map(fn ($device) => [
                    'id' => $device->id,
                    'uuid' => $device->uuid,
                    'name' => $device->name,
                ])->values(),
    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-5xl mx-auto px-4 py-4"
             x-data="phoneConfigurator(@js($seriesOptions), @js($ruleOptions))">
            <script>
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 md:p-6 mb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select x-model="selectedSeriesId" @change="onSeriesChange"
                                class="w-full border border-gray-300 rounded-xl p-3 text-base">
                            <option value="">Select Series</option>
                            <template x-for="seriesItem in seriesOptions" :key="seriesItem.id">
                                <option :value="String(seriesItem.id)" x-text="seriesItem.name"></option>
                            </template>
                        </select>
                    </div>
    ...
    <script>
        function phoneConfigurator(seriesOptions, ruleOptions) {
            return {
                ...
                whatsappAction: '#',
                onSeriesChange() {
                    const currentSeries = this.seriesOptions.find((item) => String(item.id) === String(this.selectedSeriesId));
                    this.availableModels = currentSeries ? (currentSeries.devices || []) : [];
                    this.selectedModelId = '';
                    this.resetAfterModel();
                },
                onModelChange() {
                    const model = this.availableModels.find((item) => String(item.id) === String(this.selectedModelId));
                    this.selectedModelUuid = model ? model.uuid : '';
                    this.whatsappAction = this.selectedModelUuid ? phoneWhatsappTemplate.replace('__DEVICE_UUID__', this.selectedModelUuid) : '#';
                    this.resetAfterModel();
