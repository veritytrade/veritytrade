<x-app-layout>
    <div class="premium-shell">
        <!-- Tabs Navigation -->
        <div class="sticky top-16 z-20 bg-white/90 backdrop-blur border-b border-slate-200/70 shadow-sm">
            <div class="overflow-x-auto scrollbar-hide">
                <div class="flex min-w-max px-4">
                    <a href="#hot-deals" 
                       class="block flex-shrink-0 px-5 py-4 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 to-sky-600 rounded-t-xl whitespace-nowrap shadow-sm">
                        🔥 Hot Deals
                    </a>
                    <a href="#laptops" class="block flex-shrink-0 px-5 py-4 text-sm font-medium text-slate-600 hover:text-sky-600 whitespace-nowrap transition-colors">
                        Laptops
                    </a>
                    <a href="#tablets" class="block flex-shrink-0 px-5 py-4 text-sm font-medium text-slate-600 hover:text-sky-600 whitespace-nowrap transition-colors">
                        Tablets
                    </a>
                    <a href="#consoles" class="block flex-shrink-0 px-5 py-4 text-sm font-medium text-slate-600 hover:text-sky-600 whitespace-nowrap transition-colors">
                        Consoles
                    </a>
                </div>
            </div>
        ...
        <div id="hot-deals" class="max-w-6xl mx-auto px-4 py-6">
            <h1 class="premium-title text-2xl md:text-3xl mb-6 text-center">
                <span class="bg-gradient-to-r from-emerald-600 to-sky-600 bg-clip-text text-transparent"> Today's Hot Deals</span>
            </h1>
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach($deals as $deal)
                        <div class="premium-card-soft hover:shadow-[0_14px_34px_rgba(2,6,23,0.11)] transition-all duration-300 transform hover:-translate-y-1" 
                                        ...
                                        <div x-show="images.length > 1" class="absolute bottom-4 left-0 right-0 flex justify-center space-x-2">
                                            <template x-for="(img, index) in images" :key="index">
                                                <button @click="current = index; resetAutoRotate()" 
                                                        ...
                                                        :class="current === index ? 'bg-gradient-to-r from-emerald-500 to-sky-500 scale-125' : 'bg-white/70 hover:bg-white'">
                                                </button>
                                            </template>
                                        </div>
                            ...
                            <div class="p-4">
                                {{-- Title --}}
                                <h2 class="text-base sm:text-lg font-bold text-sky-700 mb-0.5 text-center line-clamp-2">
                                    {{ $deal->title }}
                                </h2>
                                ...
                                @if($deal->price_display)
                                    @php
                                        ...
                                        $pVal = $pVal !== '' ? $pVal : $p;
                                    @endphp
                                    <div class="text-xl sm:text-2xl font-extrabold text-emerald-600 mb-3 text-center"><span>₦{{ $pVal }}</span></div>
                                @endif
                                ...
                                @if($specLinesLanding->isNotEmpty())
                                    <div class="mb-3">
                                        <button type="button"
                                                ...
                                                class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 text-sm font-medium text-sky-700 transition-colors">
                                        ...
                                        <div x-show="specsOpen"
                                            ...
                                            @foreach($specLinesLanding as $spec)
                                                <div class="bg-gradient-to-r from-sky-50 to-emerald-50 border border-gray-100 rounded-lg p-2.5 hover:shadow-md transition-all duration-200">
                                                    <div class="text-xs text-sky-700 font-semibold mb-1 uppercase tracking-wide">{{ $spec['key'] }}</div>
                                ...
                                @if(filled($deal->uuid))
                                    <a href="{{ route('deal.whatsapp', ['deal' => $deal->uuid]) }}"
                                       class="premium-btn-primary w-full min-h-12 text-sm sm:text-base rounded-xl flex py-3 space-x-2">
            ...
            {{-- Footer + Chat Now --}}
            <div class="mt-10 text-center pb-8 premium-card-soft p-6">
                <p class="text-xs text-slate-500">Secure transactions via WhatsApp only</p>
                <p class="mt-3 text-sm text-slate-600">Laptops, Tablets, Gaming Consoles, or need agency assistance for China imports?</p>
                <a href="https://wa.me/2347084117779?text=Hi%2C%20I%27m%20interested%20in%20laptops%2C%20tablets%2C%20gaming%20consoles%2C%20or%20agency%20assistance%20for%20China%20imports."
                   ...
                   class="premium-btn-primary mt-4 gap-2 px-6 py-3">
        ...
        <div id="laptops" class="max-w-6xl mx-auto px-4 py-12 border-t border-slate-200">
            <h2 class="text-xl font-bold text-slate-800 mb-4 text-center">Laptops</h2>
            <p class="text-center text-slate-500">Coming soon.</p>
        </div>
        <div id="tablets" class="max-w-6xl mx-auto px-4 py-12 border-t border-slate-200">
            <h2 class="text-xl font-bold text-slate-800 mb-4 text-center">Tablets</h2>
            <p class="text-center text-slate-500">Coming soon.</p>
        </div>
        <div id="consoles" class="max-w-6xl mx-auto px-4 py-12 border-t border-slate-200">
            <h2 class="text-xl font-bold text-slate-800 mb-4 text-center">Consoles</h2>
            <p class="text-center text-slate-500">Coming soon.</p>
        </div>
        ...
        <div class="fixed bottom-5 right-5 md:hidden z-50">
            <a href="https://wa.me/2347084117779"
               target="_blank"
               class="bg-gradient-to-r from-emerald-500 to-sky-500 hover:from-emerald-600 hover:to-sky-600 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-transform hover:scale-110">
