...
<div class="max-w-4xl mx-auto p-4 sm:p-6">
    @php
        $canManageShipmentStage = auth()->user()->hasRole('super_admin') || auth()->user()->hasPermission('update_shipment_stage');
        $logisticsLabel = [
            'skycargo' => 'SkyCargo',
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        @if($canManageShipmentStage)
            ...
            <form method="POST" action="{{ route('admin.shipments.mark-agent-collected', $shipment) }}" class="inline" onsubmit="return confirm('Mark this shipment as collected by your agent from logistics warehouse?');">
                @csrf
                <button type="submit" class="inline-flex items-center justify-center min-h-[44px] px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg">
                    Mark agent collected
                </button>
            </form>
        @endif
        </div>
